...
// Group, [HDR] FlipModel/Float/DumpComposite/UpgradeTargets.
...
static effect_uniform_variable s_hdr_flip_var = { 0 }, s_hdr_float_var = { 0 };
...
static bool s_hdr_flip_ui = false, s_hdr_float_ui = false;   // last value read back from the panel
...
// Writes the two swap chain switches back to the ini, where create_swapchain reads them next
// launch. Every other line is preserved: the hash groups are generated and must survive.
static void save_hdr_ini()
{
	const std::string path = base_path() + "BlancoVision.addon.ini";
	std::vector<std::string> lines;
	bool seen_flip = false, seen_float = false, seen_section = false;
	{
		...
		while (std::getline(in, line))
		{
			...
			if (line.rfind("FlipModel=", 0) == 0) { line = std::string("FlipModel=") + (s_hdr_flip ? "1" : "0"); seen_flip = true; }
			else if (line.rfind("Float=", 0) == 0) { line = std::string("Float=") + (s_hdr_float ? "1" : "0"); seen_float = true; }
			else if (line == "[HDR]") seen_section = true;
			lines.push_back(line);
		}
	}
	std::ofstream out(path, std::ios::trunc);
	if (!out) { logf("BlancoVision.addon hdr: cannot write %s", path.c_str()); return; }
	for (const std::string &l : lines) out << l << "\n";
	if (!seen_section) out << "\n[HDR]\n";
	if (!seen_flip) out << "FlipModel=" << (s_hdr_flip ? 1 : 0) << "\n";
	if (!seen_float) out << "Float=" << (s_hdr_float ? 1 : 0) << "\n";
	logf("BlancoVision.addon hdr: saved FlipModel=%d Float=%d, applies on the next launch", s_hdr_flip ? 1 : 0, s_hdr_float ? 1 : 0);
}
...
static void collect_rules(effect_runtime *runtime)
{
	...
	s_enable_var = { 0 }; s_dump_var = { 0 }; s_hdr_flip_var = { 0 }; s_hdr_float_var = { 0 };
	runtime->enumerate_uniform_variables(nullptr, [](effect_runtime *runtime, effect_uniform_variable var) {
		...
		if (std::strcmp(source, "hdr") == 0)
		{
			char key[32] = "";
			runtime->get_annotation_string_from_uniform_variable(var, "bv_key", key);
			if (std::strcmp(key, "FlipModel") == 0) s_hdr_flip_var = var;
			else if (std::strcmp(key, "Float") == 0) s_hdr_float_var = var;
			return;
		}
	...
	// create_swapchain already ran off the ini, so show what is in force, not what the preset restored.
	if (s_hdr_flip_var.handle != 0) runtime->set_uniform_value_bool(s_hdr_flip_var, &s_hdr_flip, 1);
	if (s_hdr_float_var.handle != 0) runtime->set_uniform_value_bool(s_hdr_float_var, &s_hdr_float, 1);
	s_hdr_flip_ui = s_hdr_flip; s_hdr_float_ui = s_hdr_float;
...
static void refresh_rules(effect_runtime *runtime, command_list *, resource_view, resource_view)
{
	...
	if (s_hdr_flip_var.handle != 0 || s_hdr_float_var.handle != 0)
	{
		bool flip = s_hdr_flip, flt = s_hdr_float;
		if (s_hdr_flip_var.handle != 0) runtime->get_uniform_value_bool(s_hdr_flip_var, &flip, 1);
		if (s_hdr_float_var.handle != 0) runtime->get_uniform_value_bool(s_hdr_float_var, &flt, 1);
		if (flip != s_hdr_flip_ui || flt != s_hdr_float_ui)
		{
			s_hdr_flip = s_hdr_flip_ui = flip; s_hdr_float = s_hdr_float_ui = flt;
			save_hdr_ini();
		}
	}
	for (Rule &r : s_rules)
